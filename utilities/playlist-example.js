...
class App extends React.Component {
  ...
  async _loadNewPlaybackInstance(playing) {
    ...
    if (PLAYLIST[this.index].isVideo) {
      ...
      try {
        // status = await this._video.getStatusAsync();
        // console.log(status);
      } catch (e) {
        // console.warn('getStatusAsync issue 1');
      ...
      try {
        ...
        try {
          ...
          console.warn('getStatusAsync issue');
          console.warn(e.message);
        }
      }
  ...
  _callback = status => {
    if (status.isLoaded) {
      ...
      if (status.isBuffering) {
        ...
        console.log('callback: loaded isBuffering');
      }
      ...
      console.log('callback not loaded');
      if (status.error) {
