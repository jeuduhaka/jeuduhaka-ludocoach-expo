...
import { KEY_PREFIX, REHYDRATE } from 'redux-persist/src/constants';
import createAsyncLocalStorage from 'redux-persist/src/defaults/asyncLocalStorage';
import purgeStoredState from 'redux-persist/src/purgeStoredState';

export default function createPersistor(store, config) {
  // defaults
  const serializer =
    config.serialize === false ? data => data : defaultSerializer;
  const deserializer =
    config.serialize === false ? data => data : defaultDeserializer;
  ...
  store.subscribe(async () => {
    ...
    //     if (
    //       (paused && len === storesToProcess.length) ||
    //       storesToProcess.length === 0
    //     ) {
      ...
      stateIterator(incoming, (subState, key) => {
        try {
          ...
          if (process.env.NODE_ENV !== 'production')
            console.warn(
              `Error rehydrating data for key "${key}"`,
              subState,
              err
            );
        }
